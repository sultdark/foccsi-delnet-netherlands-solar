...
# Installation und Aufruf des Pakete (Schritt kann ï¿½bersprungen werden, wenn Pakete bereits installiert ist)
...
# Setzen eines Arbeitsordners (Pfad: //x/y/z) in dem erzeugte Dateien (z. B. Bilder, Ergebnisse) gespeichert werden kï¿½nnen
...
e = 1                     # Endtag, letzter Tag des "rolling Window" von hinten gezï¿½hlt 
                          # (z.B. 1 bedeutet bis zum letzten Tag der Datenbasis; 2 bedeutet, bis zum vorletzten Tag der Datenbasis)

FL_d = 165                # Definition der Lï¿½nge des "rolling window" in Tagen
FX_h = 15                 # Definition des Zeitpunkts der Prognoselieferung (9 Uhr) fï¿½r die einzelnen day-ahead Prognosen
...
# Achtung: Zeitreihen mï¿½ssen bei 0:00 Uhr beginnen und bei 24:00 Uhr enden!
# Intern kann statt Server-lsew auch 141.43.85.131 gesetzt werden.
...
### Parameter fÃ¼r Datenvorbehandlung der Folgetagsprognosen B

Max.install = max(MW_J, na.rm =T)

Anzahl.unmoeglich.max = 5     # Maximale Anzahl der erlaubten "unmï¿½glichen Werte" (z. B. negative Werte) bevor die Daten verworfen werden
...
### Parameter fÃ¼r Datenvorbehandlung der historischen Daten B.hist

Anzahl.unmoeglich.hist.max = 100        # Maximale Anzahl der erlaubten "unmï¿½glichen Werte" (z. B. negative Werte) bevor die Daten verworfen werden
...
Anzahl.Wertwdh.Wert.tol.hist.max = 5    # Zï¿½hlvariable fï¿½r die Anzahl von tolerierten gleichen Werten im Datenbereich, die aufeinander folgen

Zahl.Anbieter.genutzt.tgl = 0 # Zahl der Anbieter, welche tatsï¿½chlich an einem Tag fï¿½r die Kombination genutzt wurden



### Erzeugung einen plausiblen HR-Vektors: HRN

HRN = rep(0, times=Ende) # Einen Vektor mit gleicher Lï¿½nge wie HR voller Nullen fï¿½r die bereinigte Zeitreihe

for (i in 1:Ende) # Kontrolliere die eingelesene Zeitreihe Wert fï¿½r Wert!
{
  if (!is.na(HR[i])) # Wenn der Wert eine Zahl ist...
  {
    ...
    {
      HRN[i]<-HR[i] # ...dann ï¿½bernimm diesen Wert in die bereinigte Zeitreihe!
    }
    else
    {
...
# Ergï¿½nzung der eingelesenen Daten durch Berechnung der Prognosefehlervektors der Einzelprognosen sowie des
...
FL = FL_d*96-FX_h*4       # Fï¿½nsterlï¿½nge des "rolling window" in Viertelstunden [165Tage*96=15.840 abzgl. der Viertelstunden, die zwischen Berechnungszeitpunkt (Vorliegen der Prognosen fï¿½r den Folgetag, 9 Uhr des aktuellen Tages) und Prognosebeginn (0 Uhr am Folgetag) liegen]
Fstart = (d-1) * 96 + 1   # Fensterbeginn (erste Viertelstunde des Starttages)
Fend = Fstart - 1 + FL    # Fensterende (in der Praxis spï¿½ter: letzte Viertelstunde vor Berechnungsdurchfï¿½hrung)
...
# Zahl der maximal zur Verfï¿½gung stehenden Prognosen

Zahl.Prognosen = 7         # Zahl der maximal mï¿½glichen Prognosen

# Erzeug Matrix zur Speicherung der Grï¿½nde, warum ein Anbieter nicht fï¿½r die Kombination genutzt wird
...
for (j in (FL_d+d):(Ende/96))     # j:=Tagesnummer des Tages, fï¿½r den neue Prognosen vorliegen, ab ersten Prognosetag bis Endtag
{
  Zahl.Anbieter.genutzt = 0  # Zahl der Anbieter, welche tatsï¿½chlich fï¿½r die Kombination genutzt werden
  
  
  ### Datenvektoren einlesen: Folgetagsprognosen und historische Prognosen sowie die historischen Hochrechnungen zum Schï¿½tzen der Modellkoeffizienten
  ...
  for (l in (1:Zahl.Prognosen))
  {
    ...
    #i=1
    B = Prognosen.Folgetag[,l]  # Daten von Anbieter i fï¿½r die Folgetagsprognosen --> Erzeuge einen lokalen Datenvektor fï¿½r die Daten des Tages um die Ausgangsdaten nicht zu verï¿½ndern
    B.hist = Prognosen.hist[,l] # Historische Daten von Anbieter i zur Schï¿½tzung der Modellkoeffizienten --> Erzeuge einen lokalen Datenvektor fï¿½r die Historie um die Ausgangsdaten nicht zu verï¿½ndern
    
    ### Parameter fï¿½r Datenvorbehandlung der Folgetagsprognosen B

    ### Definition der Zï¿½hlvariablen
    
    Anzahl.unmoeglich = 0         # Zï¿½hlvariable fï¿½r die Anzahl der "unmï¿½glichen Werte" (z. B. negative Werte) im aktuell betrachteten Datenbereich
    Anzahl.leer.wdh = 0           # Zï¿½hlvariable fï¿½r die Anzahl fehlender Werte im Datenbereich, die aufeinander folgen  
    Anzahl.leer.gesamt = 0        # Zï¿½hlvariable fï¿½r die Anzahl fehlender Werte im Datenbereich insgesamt
    Anzahl.Wertwdh.Null = 0       # Zï¿½hlvariable fï¿½r die Anzahl von Nullen im Datenbereich, die aufeinander folgen
    Anzahl.Wertwdh.Wert = 0       # Zï¿½hlvariable fï¿½r die Anzahl von gleichen Werten im Datenbereich, die aufeinander folgen
    ...
    ### Beginn der Datenvorbehandlung - ï¿½berprï¿½fung der Folgetagsprognosen auf Plausibiliï¿½t
    
    for (i in (1:length(B))) # Fï¿½r alle i in B
    {
      ### Prï¿½fung auf unmï¿½gliche Werte in den Daten
      
      if (!is.na(B[i])) # Wenn B[i] nicht leer ist, dann...
      {
        if (B[i] < 0 || B[i] > Max.install) {Anzahl.unmoeglich = Anzahl.unmoeglich + 1} # Wenn B[i] ein unmï¿½glicher Wert ist, dann zï¿½hle
        if (Anzahl.unmoeglich > Anzahl.unmoeglich.max) {B.hist = rep(0, times=length(B.hist)); break.var = 1; Anbieter.stat.grund[j,l] = 1; break} # Wenn Anzahl.unmoeglich einen kritischen Wert ï¿½berschreitet, ersetze B.hist durch den Null-Vektor und verlasse die "for"-Schleife
      }
      
      
      
      ### Prï¿½fung auf Wiederholungen von Werten
      
      if (i > 1 && !is.na(B[i]) && !is.na(B[i-1])) # Ab dem zweiten Wert und wenn B[i] und B[i-1] nicht leer sind, dann...
      {
        if (B[i] == 0 && B[i] == B[i-1]) # Wenn B[i] gleich Null ist und identisch mit dem Vorgï¿½nger
        {
          Anzahl.Wertwdh.Null = Anzahl.Wertwdh.Null + 1 # Zï¿½hle die Widerholungen! Achtung nicht die Anzahl der Nullen!
          if (Anzahl.Wertwdh.Null > Anzahl.Wertwdh.Null.max) {B.hist = rep(0, times=length(B.hist)); break.var = 1; Anbieter.stat.grund[j,l] = 2; break} # Wenn Anzahl.Wertwdh.Null einen kritischen Wert ï¿½berschreitet, ersetze B.hist durch den Null-Vektor und verlasse die "for"-Schleife
        }
        else {Anzahl.Wertwdh.Null = 0} # Wenn die die Folge von Nullen abbricht ohne den kritischen Wert Anzahl.Wertwdh.Null.max zu ï¿½berschreiten, setze Anzahl.Wertwdh.Null wieder auf Null
        if (B[i] !=  0 && B[i] == B[i-1]) # Wenn B[i] ungleich Null ist und identisch mit dem Vorgï¿½nger
        {
          Anzahl.Wertwdh.Wert = Anzahl.Wertwdh.Wert + 1 # Zï¿½hle die Widerholungen! Achtung nicht die Anzahl der gleichen Werte!
          if (Anzahl.Wertwdh.Wert > Anzahl.Wertwdh.Wert.max) {B.hist = rep(0, times=length(B.hist)); break.var = 1; Anbieter.stat.grund[j,l] = 3; break} # Wenn Anzahl.Wertwdh.Wert einen kritischen Wert ï¿½berschreitet, ersetze B.hist durch den Null-Vektor und verlasse die "for"-Schleife
        }
        else {Anzahl.Wertwdh.Wert = 0}# Wenn die die Folge von gleichen Werten abbricht ohne den kritischen Wert Anzahl.Wertwdh.Wert.hist.max zu ï¿½berschreiten, setze Anzahl.Wertwdh.Wert.hist wieder auf Null
      }
      ...
      ### Prï¿½fung auf die Hï¿½ufigkeit der Leerstellen insgesamt und wieviele aufeinander folgen
      
      if (is.na(B[i])) # Wenn B[i] leer ist, dann...
      {
        Anzahl.leer.wdh = Anzahl.leer.wdh + 1 # Zï¿½hle die Anzahl der sich wiederholenden Leerstellen
        Anzahl.leer.gesamt = Anzahl.leer.gesamt + 1 # Zï¿½hle die Anzahl der Leerstellen
        if (Anzahl.leer.wdh > Anzahl.leer.wdh.max || Anzahl.leer.gesamt > Anzahl.leer.gesamt.max) {B.hist = rep(0, times=length(B.hist)); break.var = 1; Anbieter.stat.grund[j,l] = 4; break} # Wenn Anzahl.leer.wdh bzw. Anzahl.leer.gesamt einen kritischen Wert ï¿½berschreitet, ersetze B.hist durch den Null-Vektor und verlasse die "for"-Schleife
      }
      else {Anzahl.leer.wdh = 0} # Wenn die Folge der NA durch einen Wert abgebrochen wird, setze Anzahl.leer.wdh auf Null zurï¿½ck
    }
    
    
    
    
    ### Aufbereitung der Prognosen - Entfernung der unmï¿½glichen Werte und fehlender Werte in den Prognosen
    
    if (break.var == 0) # Wenn die Daten die Plausibilitï¿½tsprï¿½fung ï¿½berstanden haben, dann
    {
      ### Ersetzung von unplausiblen Werten
      
      for (i in (1:length(B))) # Fï¿½r alle Werte in B
      {
        if (!is.na(B[i])){if (B[i] < 0 || B[i] > Max.install) {B[i] = NA}} # Suche fï¿½r alle nichtleeren Werte nach unmï¿½glichen Werten und setze sie NA
      }
      #B = tsclean(B, replace.missing = TRUE, lambda = NULL) # Entferne alle Ausreiï¿½er und interpoliere alle NA-Werte
    ...
    ### Aufbereitung der historischen Daten des Anbieters mit "Einschwingen" als Spezialfall (Setze dazu die "xxx.max"-Parameter auf sehr groï¿½e Werte)
    
    ### Definition der Zï¿½hlvariablen
    
    Anzahl.unmoeglich.hist = 0         # Zï¿½hlvariable fï¿½r die Anzahl der "unmï¿½glichen Werte" (z. B. negative Werte) im aktuell betrachteten Datenbereich
    Anzahl.leer.wdh.hist = 0           # Zï¿½hlvariable fï¿½r die Anzahl fehlender Werte im Datenbereich, die aufeinander folgen  
    Anzahl.leer.gesamt.hist = 0        # Zï¿½hlvariable fï¿½r die Anzahl fehlender Werte im Datenbereich insgesamt
    Anzahl.Wertwdh.Null.hist = 0       # Zï¿½hlvariable fï¿½r die Anzahl von Nullen im Datenbereich, die aufeinander folgen
    Anzahl.Wertwdh.Wert.hist = 0       # Zï¿½hlvariable fï¿½r die Anzahl von gleichen Werten im Datenbereich, die aufeinander folgen
    ...
    ### Beginn der Datenvorbehandlung - ï¿½berprï¿½fung der historischen Daten auf Plausibiliï¿½t
    
    if (break.var == 0)  # Wenn die Folgetagsprognosen die Plausibilitï¿½tsprï¿½fung ï¿½berstanden haben, dann
    {
      for (i in (1:length(B.hist))) # Fï¿½r alle i in B.hist
      {
        ### Prï¿½fung auf unmï¿½gliche Werte in den Daten
        
        if (!is.na(B.hist[i])) # Wenn B.hist[i] nicht leer ist, dann...
        {
          if (B.hist[i] < 0 || B.hist[i] > Max.install) {Anzahl.unmoeglich.hist = Anzahl.unmoeglich.hist + 1} # Wenn B.hist[i] ein unmï¿½glicher Wert ist, dann zï¿½hle
          if (Anzahl.unmoeglich.hist > Anzahl.unmoeglich.hist.max) {B.hist = rep(0, times=length(B.hist)); break.var.hist = 1; Anbieter.stat.grund[j,l] = 5; break} # Wenn Anzahl.unmoeglich einen kritischen Wert ï¿½berschreitet, ersetze B.hist durch den Null-Vektor und verlasse die "for"-Schleife
        }
        ...
        {
          if (B.hist[i] == 0 && B.hist[i] == B.hist[i-1]) # Wenn B.hist[i] gleich Null ist und identisch mit dem Vorgï¿½nger
          {
            Anzahl.Wertwdh.Null.hist = Anzahl.Wertwdh.Null.hist + 1 # Zï¿½hle die Widerholungen! Achtung nicht die Anzahl der Nullen!
            if (Anzahl.Wertwdh.Null.hist > Anzahl.Wertwdh.Null.hist.max) {B.hist = rep(0, times=length(B.hist)); break.var.hist = 1; Anbieter.stat.grund[j,l] = 6; break} # Wenn Anzahl.Wertwdh.Null.hist einen kritischen Wert ï¿½berschreitet, ersetze B.hist durch den Null-Vektor und verlasse die "for"-Schleife
          }
          else {Anzahl.Wertwdh.Null.hist = 0} # Wenn die die Folge von Nullen abbricht ohne den kritischen Wert Anzahl.Wertwdh.Null.hist.max zu ï¿½berschreiten, setze Anzahl.Wertwdh.Null.hist wieder auf Null
          if (B.hist[i] !=  0 && B.hist[i] == B.hist[i-1]) # Wenn B.hist[i] ungleich Null ist und identisch mit dem Vorgï¿½nger
          {
            Anzahl.Wertwdh.Wert.hist = Anzahl.Wertwdh.Wert.hist + 1 # Zï¿½hle die Widerholungen! Achtung nicht die Anzahl der gleichen Werte!
            if (Anzahl.Wertwdh.Wert.hist > Anzahl.Wertwdh.Wert.hist.max) {B.hist = rep(0, times=length(B.hist)); break.var.hist = 1; Anbieter.stat.grund[j,l] = 7; break} # Wenn Anzahl.Wertwdh.Wert.hist einen kritischen Wert ï¿½berschreitet, ersetze B.hist durch den Null-Vektor und verlasse die "for"-Schleife
          }
          else {if (Anzahl.Wertwdh.Wert.hist > Anzahl.Wertwdh.Wert.tol.hist.max) {B.hist[(i-Anzahl.Wertwdh.Wert.hist-1):(i-1)] = 0}; Anzahl.Wertwdh.Wert.hist = 0}# Wenn die die Folge von gleichen Werten abbricht ohne den kritischen Wert Anzahl.Wertwdh.Wert.hist.max zu erreichen aber die Anzahl an tolerierten Wiederholungen ï¿½berschreitet, ersetze die Werte die gleich waren durch Null und setze dann Anzahl.Wertwdh.Null.hist wieder auf Null
        }
        else
        {
          if (i > 1 && Anzahl.Wertwdh.Wert.hist > Anzahl.Wertwdh.Wert.tol.hist.max)
          {
            B.hist[(i-Anzahl.Wertwdh.Wert.hist-1):(i-1)] = 0; Anzahl.Wertwdh.Wert.hist = 0 # Wenn die Folge gleicher Werte durch ein NA abgebrochen wird und die Anzahl an tolerierten Wiederholungen ï¿½berschritten wurde, ersetze die Werte die gleich (!=0) waren durch Null und setze dann Anzahl.Wertwdh.Wert.hist wieder Null
          }
        ...
        {
          Anzahl.leer.wdh.hist = Anzahl.leer.wdh.hist + 1 # Zï¿½hle die Anzahl der sich wiederholenden Leerstellen
          Anzahl.leer.gesamt.hist = Anzahl.leer.gesamt.hist + 1 # Zï¿½hle die Anzahl der Leerstellen
          if (Anzahl.leer.wdh.hist > Anzahl.leer.wdh.hist.max || Anzahl.leer.gesamt.hist > Anzahl.leer.gesamt.hist.max) {B.hist = rep(0, times=length(B.hist)); break.var.hist = 1; Anbieter.stat.grund[j,l] = 8; break} # Wenn Anzahl.leer.wdh.hist bzw. Anzahl.leer.gesamt.hist einen kritischen Wert ï¿½berschreitet, ersetze B.hist durch den Null-Vektor und verlasse die "for"-Schleife
        }
        else {Anzahl.leer.wdh.hist = 0} # Wenn die Folge der NA durch einen Wert abgebrochen wird, setze Anzahl.leer.wdh.hist auf Null zurï¿½ck
      }
    }
    
    ### Aufbereitung der Historie - Entfernung der unmï¿½glichen Werte und fehlender Werte in den Prognosen
    
    if (break.var.hist == 0) # Wenn die historischen Daten die Plausibilitï¿½tsprï¿½fung ï¿½berstanden haben, dann
    {
      ### Ersetzung von unplausiblen Werten
      
      for (i in (1:length(B.hist))) # Fï¿½r alle Werte in B
      {
        if (!is.na(B.hist[i])){if (B.hist[i] < 0 || B.hist[i] > Max.install) {B.hist[i] = NA}} # Suche fï¿½r alle nichtleeren Werte nach unmï¿½glichen und setze sie NA
      }
      #B.hist = tsclean(B.hist, replace.missing = TRUE, lambda = NULL) # Entferne alle Ausreiï¿½er und interpoliere alle NA-Werte
    ...
    ### Speichern, ob der Anbieter berï¿½cksichtigt wurde oder nicht
    
    if (break.var == 0 && break.var.hist == 0) {Zahl.Anbieter.genutzt = Zahl.Anbieter.genutzt + 1}
    
    ### Speichern der ï¿½berarbeiteten Anbieterdaten (Historische Prognosen und Folgetagsprognosen)
    
    Prognosen.Folgetag[,l] = B # Daten von Anbieter i fï¿½r die Folgetagsprognosen --> Erzeuge einen lokalen Datenvektor fï¿½r die Daten des Tages um die Ausgangsdaten nicht zu verï¿½ndern
    Prognosen.hist[,l] = B.hist # Historische Daten von Anbieter i zur Schï¿½tzung der Modellkoeffizienten --> Erzeuge einen lokalen Datenvektor fï¿½r die Historie um die Ausgangsdaten nicht zu verï¿½ndern
  ...
  # Anwendung des Kombinationsmodells "cross validation" cvKM mit Konfiguration des Alpha-Parameters und des Fehlermaï¿½es
  ...
  # Erstellung der BTU-Prognose der 96 Viertelstunden fï¿½r den Tag j
  
  r = 0
  
  for (k in ((j-1)*96+1):(j*96) )   # k:= Viertelstundennummer des Tages j, von 1 bis 96 ###
  {
    # fï¿½r den Tag j relevante Prognosen; dabei wieder temporï¿½re Ersetzung zu unplausibler Anbieter durch fiktiven Anbieter 0
    # hier noch ï¿½berlegen: 1.) Muss A0N hier aufgefï¿½hrt werden? 2.) Mï¿½ssen Unplausible hier extra aussortiert werden (erhalten ja eh kein Gewicht oder)?
...
for (i in Pstart:Ende) { #i:Viertelstundenummer Zahl.Anbieter.genutzt.tgl ist jetzt vierstï¿½ndlich
...
### Ausgabe: Ergebnistabellen, Fehlermaï¿½e, Grafiken
### -----------------------------------------------
...
# Berechnung der Fehlermaï¿½e: mittlerer quadratischer Fehler (MSE),
# mittlerer absoluter Fehler (MAE) und mittlerer Fehler (ME)

# Berechnen der Gesamtfehlermaï¿½e
...
# Speichern der Gesamtfehlermaï¿½e
...
# Berechnen der Jahresfehlermaï¿½e
...
# Berechnen der durchschnittlichen viertelstï¿½ndlichen Einspeiseleistung je Jahr (HR oder HRN benutzen)
...
# Speichern der Jahresehlermaï¿½e

Fehlermaï¿½e_J = matrix(c(JLis[1:JAnz],
                        MW_J[1:JAnz],
                        JMW[1:JAnz],
...
# Grafik 4v: Prognosefehler der BTU-Prognose, viertelstssndlich (Einhï¿½llende)
#
# lokale Maxima (obere Einhï¿½llende) [oder doch lieber monatlich?]
#
...
#}
#
# lokale Minima (untere Einhï¿½llende)
#
...
# Grafik 4: Prognosefehler der BTU-Prognose, monatlich (Einhï¿½llende)
...
# Darstellung der Grï¿½nde, warum ein Anbieter in der Kombination ignoriert wird
...
# Darstellung der tï¿½glich genutzten Anbieter
...
# # Code fï¿½r png-Export der Gewichte (von Dragana)
...
# Grafik 6: Methodenvergleich, jï¿½hrlich
...
par(mfrow=c(1,1))
plot(BTU)
plot(Vergleich_J, type="o", col="blue", lwd=2, xlab="Jahr", ylab="Verschlechterung [%]", main = "jï¿½hrlicher Vergleich (mse_BTU - mse_K) / mse_K")
